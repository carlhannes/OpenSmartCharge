...
export default function BalancerCard({ name, mainBreakerA }: Props) {
  ...
  const allocData = alloc ? Object.entries(alloc).map(([id, amps]) => ({ id, amps })) : []
    ...
    <div className={styles.card}>
      ...
      <div className={styles.freeAmps}>
        ...
        <span className={styles.freeValue}>
          {state?.freeAmps != null ? `${state.freeAmps} A` : '—'}
        </span>
      ...
      {allocData.length === 0 && (
        <p style={{ color: 'var(--color-muted)', fontSize: '0.875rem' }}>No allocations yet.</p>
      )}
    </div>
  )
}
