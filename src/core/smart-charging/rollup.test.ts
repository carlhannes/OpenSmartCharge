...
test('records the per-hour MAX; worstCaseLoadA spans the last N days for that hour-of-day', () => {
  withDb((db) => {
    recordHouseholdLoad(db, at(1), 10, TZ)
    recordHouseholdLoad(db, at(1), 5, TZ) // same day+hour → stays the max (10)
    recordHouseholdLoad(db, at(2), 20, TZ)
    recordHouseholdLoad(db, at(3), 15, TZ)
    recordHouseholdLoad(db, at(4), 5, TZ) // "today"

    // last 3 days (Jul 2–4) worst at hour 2 → 20
    expect(worstCaseLoadA(db, at(4), 3, TZ)).toBe(20)
    // last 2 days (Jul 3–4) → 15
    expect(worstCaseLoadA(db, at(4), 2, TZ)).toBe(15)
    // a different hour has no history → null
    expect(worstCaseLoadA(db, new Date('2026-07-04T10:00:00Z'), 3, TZ)).toBeNull()
  })
})
...
test('DST fall-back: the twice-occurring local hour collapses into one max row', () => {
  withDb((db) => {
    ...
    recordHouseholdLoad(db, new Date('2026-10-25T00:30:00Z'), 8, TZ)
    recordHouseholdLoad(db, new Date('2026-10-25T01:30:00Z'), 12, TZ)
    const rows = db
      .prepare(
...
test('pruneHouseholdLoad drops rows older than keepDays', () => {
  withDb((db) => {
    recordHouseholdLoad(db, at(1), 10, TZ)
    recordHouseholdLoad(db, at(4), 20, TZ)
    pruneHouseholdLoad(db, at(4), 2, TZ) // keep Jul 3–4 → Jul 1 dropped
    expect(worstCaseLoadA(db, at(4), 30, TZ)).toBe(20) // only Jul 4 remains for hour 2
  })
})

test('ignores non-finite / negative samples', () => {
  withDb((db) => {
    recordHouseholdLoad(db, at(4), Number.NaN, TZ)
    recordHouseholdLoad(db, at(4), -5, TZ)
    expect(worstCaseLoadA(db, at(4), 3, TZ)).toBeNull()
  })
})
