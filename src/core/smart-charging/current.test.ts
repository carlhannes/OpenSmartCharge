...
test('static-tod rung: night = breaker − margin, day = breaker × fraction', () => {
  expect(
    resolveCurrentBudget({ now: nightNow, maxCurrentA: 32, mainBreakerA: 16, nightWindow: NIGHT }),
  ).toMatchObject({
    value: 13, // 16 − 3
    source: 'static-tod',
  })
  expect(
    resolveCurrentBudget({ now: dayNow, maxCurrentA: 32, mainBreakerA: 16, nightWindow: NIGHT })
      .value,
  ).toBe(8) // 16 × 0.5
})
...
test('a budget below the 6 A IEC minimum resolves to 0, never rounded up', () => {
  ...
  expect(
    resolveCurrentBudget({ now: dayNow, maxCurrentA: 16, mainBreakerA: 10, nightWindow: NIGHT })
      .value,
  ).toBe(0)
})
