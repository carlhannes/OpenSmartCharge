...
export function upsertVehicleCache(
  ...
  db
    ...
    .run(name, data.soc, data.batteryCapacityKWh ?? null, data.range ?? null, data.isCharging ? 1 : 0)
}
...
export function loadVehicleCache(db: DatabaseSync, name: string): VehicleData | null {
  const row = db.prepare('SELECT * FROM vehicle_cache WHERE vehicle_name = ?').get(name) as
    | { soc: number; battery_capacity_kwh: number | null; range_km: number | null; is_charging: number; fetched_at: string }
    | undefined
  if (!row) return null
  return {
    soc: row.soc,
